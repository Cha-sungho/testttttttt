    ...
    switch (can_msg[channel].tx.seq)
    {
        case can_tx_seq_send:
            ...
            if (can_msg[channel].tx.timer >= TX_INTERVAL) {
                ...
                if (can_msg[channel].tx.sen_flag == TRUE) drv_WriteCAN(drv_can_msg.obj[TX_ID_AA + (OBJ_MAX_1CH * channel)].obj_id, can_msg[channel].tx.buf, 8);
                else drv_WriteCAN(drv_can_msg.obj[TX_ID_100 + (OBJ_MAX_1CH * channel)].obj_id, can_msg[channel].tx.buf, 8);
            }
...
static Uint16 app_CommCanOpMsgSeqComplete(Uint16 channel)
{
    ...
    switch (can_msg[channel].tx.seq)
    {
        case can_tx_seq_complete:
            ...
            if (can_msg[channel].rx.exp_cmd == NULL || (can_msg[channel].rx.cmd) == (can_msg[channel].rx.exp_cmd)) complete++;

            if (can_msg[channel].rx.exp_op == NULL || ((can_msg[channel].rx.op & 0xf0) == (can_msg[channel].rx.exp_op & 0xf0)) || ((can_msg[channel].rx.op & 0xf0) == ((can_msg[channel].rx.exp_op & 0xf0) + 0x10))) complete++;
...
static Uint16 app_CommCanOpMainProc(Uint16 channel)
{
    if (can_msg[channel].tx.seq != can_tx_seq_ready && can_msg[channel].tx.seq != can_tx_seq_complete && can_msg[channel].rx.alarm.flag == TRUE) return 0;
    ...
    switch (can_msg[channel].op.state)
    {
        ...
        case can_op_cha_dis:
            ...
            app_CommCanOpMsgSeqReady(cmd_recipe, remap_cmd_tx_rx[can_msg[channel].remap_idx.cmd].can_rx, remap_op_tx_rx[can_msg[channel].remap_idx.op].can_rx, FALSE, channel);
            if (app_CommCanOpMsgSeqComplete(channel) == 2) {
...
static Uint16 app_CommCanOpUiEvt(Uint16 channel)
{
    ...
    if (can_msg[channel].ui_evt.start.cnt != ui_msg_rx.recv.start[channel].cnt) {
        if (can_msg[channel].ui_evt.start.flag == FALSE && (can_msg[channel].ui_evt.start.ignore_flag == FALSE)) {
            can_msg[channel].ui_evt.start.flag = TRUE;
...
static void app_CommCanOpPowerUpdateVar(Uint16 channel)
{
    ...
    can_msg[channel].rx.capa = (Uint32) ((can_msg[channel].current * (1.0 / 3600.0) * 0.1) * 10000000000);  // * 10^10, "rx.capa" value is too small.
    ...
    //Watt log
    can_msg[channel].cumulative_watt = (Uint32) ((can_msg[channel].rx.vol * can_msg[channel].rx.curr) * (1.0 / 3600.0) * 0.1);  //[Wh], Cumulative watt during 100ms.
